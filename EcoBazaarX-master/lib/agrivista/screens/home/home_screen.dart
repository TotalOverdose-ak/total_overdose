...
class _CropGridState extends State<_CropGrid> with TickerProviderStateMixin {
  static const int _initialCount = 6; // 2 rows of 3
  bool _showAll = false;

  // Staggered entrance
  late AnimationController _entranceController;
  // Wobble + bounce on tap
  int? _tappedIndex;
  late AnimationController _bounceController;
  late Animation<double> _bounceAnim;
  late Animation<double> _wobbleAnim;
  // See All arrow rotation
  ...
  void initState() {
    ...
    _bounceAnim =
        TweenSequence<double>([
          ...
          TweenSequenceItem(tween: Tween(begin: 1.06, end: 0.96), weight: 20),
          TweenSequenceItem(tween: Tween(begin: 0.96, end: 1.0), weight: 30),
        ]).animate(
          CurvedAnimation(parent: _bounceController, curve: Curves.easeInOut),
        );
    // Wobble rotation: left → right → left → center
    _wobbleAnim =
        TweenSequence<double>([
          TweenSequenceItem(tween: Tween(begin: 0.0, end: -0.04), weight: 15),
          TweenSequenceItem(tween: Tween(begin: -0.04, end: 0.04), weight: 25),
          TweenSequenceItem(tween: Tween(begin: 0.04, end: -0.025), weight: 25),
          TweenSequenceItem(
            tween: Tween(begin: -0.025, end: 0.015),
            weight: 20,
          ),
            ...
            itemBuilder: (_, i) {
              ...
              return AnimatedBuilder(
                ...
                builder: (context, child) {
                  ...
                  final wobble = isTapped ? _wobbleAnim.value : 0.0;
